...
import iu.edu.cs.p532.pair13.acquire.player.components.Move;
...
public class PlaceTileManager implements GameParameters {
	...
	public static PState PlaceTile(PState placeState, String row, int col)throws GameException{
		int rowIndex = rows.inverse().get(row);
		int colIndex = col;
		int freeCount = 0, hotelCount = 0;
		Tile[] Adj;
		PState state = new PState(placeState);
		...
		if(freeCount == 4){
			...
			move.setPlaceTile(new Tile (row,col,"singleton",1));
			return state;
		}
		//founding scenario
		if(hotelCount == 0)
	...
	public static PState PlaceTile(PState placeState, String row, int col, String label) throws GameException{
		int rowIndex = rows.inverse().get(row);
		int colIndex = col;
		int freeCount = 0, hotelCount = 0;
		PState state = new PState(placeState);
		...
		if(hotelCount == 0){
			...
			PStateManager.addSharesToCurrentPlayer(state, label);
			cPlayer.removeTileByRowCol(row, col);
			Move move = state.getMove();
			move.setPlaceTile(new Tile (row,col,label,1));
			return state;		
		}
		if(hotelCount == 1){
		...
		if(hotelCount == 2){
			for(int i=0;i<4;i++){
				if(Board.isHotel(Adj[i].getStatus())){
					if(Adj1 == null){
	...
	public static void GiveAwayBonus(PState state, Hotel hotel) throws GameException{
	...
	public static void sellStocks(PState state, Hotel hotel) throws GameException{
